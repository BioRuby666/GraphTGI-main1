...
def load_data(directory):
    ...
    TFSM_1 = np.loadtxt('./data/drug_jsim.txt', delimiter=",")     # 药物化学结构相似性矩阵
    tgSM_1 = np.loadtxt('./data/target_jsim.txt', delimiter=",")   # 靶标序列相似性矩阵

    tg_TF_SM_1 = np.loadtxt('./data/mat_protein_drug.txt')  # 药物靶标相互作用的二元稀疏矩阵
    TF_tg_SM_1 = tg_TF_SM_1.T                               # .T转置 靶标与药物相互作用矩阵

    # seq_similarity as feature
    TFSM_2 = np.loadtxt('./data/Similarity_Matrix_Drugs.txt')   # 药物之间相似性矩阵
    tgSM_2 = np.loadtxt('./data/Similarity_Matrix_Proteins.txt')  # 靶标之间相似性矩阵
    TF_tg_SM_2 = TF_tg_SM_1      # 靶标与药物相互作用矩阵
    tg_TF_SM_2 = TF_tg_SM_2.T    # 药物与靶标相互作用矩阵
...
def sample(directory, random_seed):
    all_associations = pd.read_csv('./data/interaction.csv', names=['TF', 'tg', 'label'])
    # 选择label列中取值为1的行（也就是已知关联），存储到known_associations
    ...
    sample_df.reset_index(drop=True, inplace=True)  # drop为True表示删除索引列  inplace为True表示直接在原对象上修改

    #all_associations.reset_index(drop=True, inplace=True)

    return sample_df.values   #每一行代表一个样本，分别代表药物、靶标、标签
